...
const TODOHUB_LABEL = { name: 'todohub', description: 'todohub control issue', color: '1D76DB' }
...
export class TodohubAdminIssue {
  ...
  private constructor(existingIssue?: {body: string, number: number, isClosed: boolean}) {
    if (existingIssue) {
  ...
  private compose() {
    // TODO implement
    this.midTag = '\n### Tracked Issues:'
    for (const [issueNr, trackedIssue] of Object.entries(this.data.decodedData)) {
  ...
  async write(repo: Repo) {
    ...
    if (this.existingIssueNumber) {
      return repo.updateIssue(this.existingIssueNumber, undefined, this.compose())
    }
  ...
  private parseContent(commentBody: string) {
    ...
    return parsed.groups as {preTag: string, data: string, midTag: string, postTag: string}
  }
  ...
  static async get(repo: Repo) {
    ...
    if (issue) {
      ...
      return new TodohubAdminIssue({body: issue.body || '', number: issue.number, isClosed: issue.state === 'closed'})
    }
